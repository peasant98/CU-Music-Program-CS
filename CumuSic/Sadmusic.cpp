...
Sadmusic::Sadmusic()
{
    ...
    // constructor for Sadmusic
    cout << "Welcome to the Sadmusic class. This will be more exciting than the Miscellaneous music class. You can generate random music here." << endl;
...
int Sadmusic::createMusic(string basis)
{
    ...
    int amount;
    cout << "Now a random piece of music will be created with the Sadmusic software." << endl;
    ...
    for (int i =0; i<70; i++)
    {
        ...
        if (basis == notes[i])
        {
            ...
            // makes the start index i, and all of the other midi values are based on the start midi and the theory of the minor scale.
            // each midi is a set value from the beginning midi because that will change the note. The values of 2,3, 5, 7, etc. will create the
            // sounds that can be heard as a minor scale.
            beginmidi = midi[i];
            startindex = i;
    ...
    // The minor scale arrays will have eight notes, and they are based on the index of the note and notefreq array.
    minor[0] = notes[startindex];
    minorfreq[0] = notesfreq[startindex];

    minor[1] = notes[secondindex];
    minorfreq[1] = notesfreq[secondindex];

    minor[2] = notes[thirdindex];
    minorfreq[2] = notesfreq[thirdindex];

    minor[3] = notes[fourthindex];
    minorfreq[3] = notesfreq[fourthindex];

    minor[4] = notes[fifthindex];
    minorfreq[4] = notesfreq[fifthindex];

    minor[5] = notes[sixthindex];
    minorfreq[5] = notesfreq[sixthindex];

    minor[6] = notes[seventhindex];
    minorfreq[6] = notesfreq[seventhindex];

    minor[7] = notes[finalindex];
    minorfreq[7] = notesfreq[finalindex];
    // Will play the scale.
    for (int i = 0; i<8; i++)
    {

        Beep(minorfreq[i], 500);
    }
    ...
    for (int i=0; i<amount; i++)
    {
        ...
        cout<< "The note is: " <<minor[random_x]<< endl;
        Beep(minorfreq[random_x], 60000/tempo);
        ...
        for (int b=0; b<1000; b++)
        {
            ...
            {
                madenotes[b] = minor[random_x];
                madenotesfreq[b] = minorfreq[random_x];
                break;
...
int Sadmusic::upOctave(string basis)

{
    ...
    int amount;
    cout << "Now a random piece of music will be created with the Sasmusic software. This time we will play an octave up" << endl;
    cout << "How many notes do you want to be played: " << endl;
    cin >> amount;
    int beginmidi;
    int startindex;
    ...
    for (int i=0; i<amount; i++)
    {
        ...
        random_x = rand() % 8;
        cout<< "The note is: " <<minor[random_x]<< " up an octave" <<  endl;
        Beep(minorfreq[random_x] *2, 60000/tempo);
        ...
        for (int b=0; b<1000; b++)
        {
            ...
            if (madenotes[b] == "" && madenotesfreq[b] == 0.0)
            {
                ...
                madenotes[b] = minor[random_x] + " up an octave.";
                madenotesfreq[b] = minorfreq[random_x]*2;
...
int Sadmusic::fasterSadmusic(int newtempo)
{
    int amount;
    string basis;

    cout << "Now, sad music will be played at " << newtempo << " beats per minute." << endl;
    cout << "How many notes do you want to be played? " << endl;
    cin >> amount;
    cout << "What minor scale?" << endl;
    // asks for key
    cin >> basis;
    ...
    for (int i=0; i<amount; i++)
    {
        ...
        random_x = rand() % 8;
        cout<< "The note is: " << minor[random_x]<< endl;
        Beep(minorfreq[random_x], 60000/newtempo);
...
void Sadmusic::setTempo(int newtempo)
{
    tempo = newtempo;
...
string Sadmusic::ChangeKey(string basis)
{


    cout << "The key will now be changed to the key of " << basis << " minor." << endl;
    int beginmidi;
    int startindex;
    int secondindex;
    int thirdindex;
...
void Sadmusic::playCool()
{
    ...
    for (int i=0; i<motif; i++)
    {
        ...
        for (int b=0; b<1000; b++)
        {
            ...
            {
                motifnotes[b] = minor[random_x];
                motifnotesfreq[b] = minorfreq[random_x];
                break;
            }
        }
    ...
    for (int i =0; i<motif; i++)
    {
        ...
        for (int b =0; b<8; b++)
        {
            if (motifnotesfreq[i] == minorfreq[b])
            {
                if (b == 7)
                {
                    Beep(motifnotesfreq[i], 60000/tempo);
                    Beep(minorfreq[0], 30000/tempo);
                }
                else
                {
                    Beep(motifnotesfreq[i], 60000/tempo);

                    Beep(minorfreq[b+1], 30000/tempo);
                }
        ...
        for (int b =0; b<8; b++)
        {
            if (motifnotesfreq[i] == minorfreq[b])
            {
                ...
                else
                {
                    ...
                    Beep(minorfreq[b+1], 30000/tempo);
                    Beep(minorfreq[b+2], 30000/tempo);
                }
